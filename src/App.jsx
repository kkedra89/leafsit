...
const TRANSLATIONS = {
  pl: {
    ...
    'auth.phonePlaceholder': 'Numer telefonu',
    'auth.checkEmailToConfirm': 'Konto utworzone! Sprawdź swoją skrzynkę e-mail i kliknij link potwierdzający, aby móc się zalogować.',
  ...
  en: {
    ...
    'auth.checkEmailToConfirm': 'Account created! Check your inbox and click the confirmation link before signing in.',
  ...
  uk: {
    ...
    'auth.phonePlaceholder': 'Номер телефону',
    'auth.checkEmailToConfirm': 'Обліковий запис створено! Перевірте пошту та натисніть посилання підтвердження, щоб увійти.',
...
function AuthScreen({ referralCodeFromUrl }) {
  ...
  const canSubmit = mode === 'login' ? (email && password) : (name && phone && email && password && termsAccepted);
  ...
  return (
    <div style={{ flex: 1, padding: 28, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      ...
      {mode === 'signup' && (
        <TextField icon={Phone} type="tel" placeholder={t('auth.phonePlaceholder')} value={phone} onChange={e => setPhone(e.target.value)} />
      )}
      {mode === 'signup' && (
